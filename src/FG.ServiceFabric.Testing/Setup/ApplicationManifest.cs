...
namespace FG.ServiceFabric.Testing.Setup
{
    public class ApplicationParameters
    {
        ...
        public static ApplicationParameters Load(string applicationParametersPath)
        {
            ...
            var deploymentParameters = xdocParameters.Element(xns + "Application").Element(xns + "Parameters")
                .Elements(xns + "Parameter")
    ...
    internal class ApplicationManifest
    {
        ...
        private static string GetStringParameterValue(string unresolvedValue,
            IReadOnlyDictionary<string, string> defaultValues, IReadOnlyDictionary<string, string> deploymentValues)
        {
            if (unresolvedValue.Matches(@"\[[^\]]*\]", StringComparison.InvariantCulture, false))
            {
        ...
        private static int GetIntParameterValue(string unresolvedValue,
            IReadOnlyDictionary<string, string> defaultValues, IReadOnlyDictionary<string, string> deploymentValues)
        {
            ...
            throw new ArgumentOutOfRangeException(
                $"Value {unresolvedValue}/{resolvedValue} could not be resolved as an int");
        }

        private static long GetLongParameterValue(string unresolvedValue,
            IReadOnlyDictionary<string, string> defaultValues, IReadOnlyDictionary<string, string> deploymentValues)
        {
            var resolvedValue = GetStringParameterValue(unresolvedValue, defaultValues, deploymentValues);
            if (long.TryParse(resolvedValue, out var value))
                return value;
            throw new ArgumentOutOfRangeException(
                $"Value {unresolvedValue}/{resolvedValue} could not be resolved as an long");
        }

        private static int? GetIntValue(XElement element, string attributeName,
            IReadOnlyDictionary<string, string> defaultParameters,
            IReadOnlyDictionary<string, string> deploymentParameters)
        {
        ...
        private static long? GetLongValue(XElement element, string attributeName,
            IReadOnlyDictionary<string, string> defaultParameters,
            IReadOnlyDictionary<string, string> deploymentParameters)
        {
            var attributeValue = element.Attribute(attributeName)?.Value;
        ...
        public static ApplicationManifest Load(string applicationManifestPath,
            ApplicationParameters applicationParameters)
        {
            ...
            var getIntValue = (Func<XElement, string, int?>) ((element, attributeName) =>
                GetIntValue(element, attributeName, defaultParameters, applicationParameters.Values));
            var getLongValue = (Func<XElement, string, long?>) ((element, attributeName) =>
                GetLongValue(element, attributeName, defaultParameters, applicationParameters.Values));
            ...
            foreach (var serviceManifestImportElement in serviceManifestImportElements)
            {
                ...
                var configOverrideSettingsSectionElements = serviceManifestImportElement
                    .Elements(xns + "ConfigOverrides")
                    ?.Elements(xns + "ConfigOverride").Where(element => element.Attribute("Name")?.Value == "Config")
                    ?.Elements(xns + "Settings")?.Elements(xns + "Section");

                var serviceManifestImportConfigSections =
                    new List<ApplicationManifestServiceManifestImport.ConfigSection>();
                foreach (var sectionElement in configOverrideSettingsSectionElements)
                {
                    ...
                    var parameters = sectionElement.Elements(xns + "Parameter").Select((parameterElement, i) =>
                        new KeyValuePair<string, string>(
                            ...
                            GetStringParameterValue(parameterElement.Attribute("Value")?.Value ?? "", defaultParameters,
                                applicationParameters.Values)));
                ...
                var serviceManifestImport = new ApplicationManifestServiceManifestImport
                {
                    Name = name,
                    Version = version,
                    Sections = serviceManifestImportConfigSections.ToArray()
                };
            ...
            var defaultServiceElements = manifestElement.Element(xns + "DefaultServices")?.Elements(xns + "Service") ??
                                         new XElement[0];
            ...
            foreach (var defaultServiceElement in defaultServiceElements)
            {
                ...
                applicationManifestDefaultService.InstanceCount = applicationManifestDefaultService.InstanceCount == -1
                    ? 10
                    : applicationManifestDefaultService.InstanceCount;
                applicationManifestDefaultService.TargetReplicaSetSize =
                    getIntValue(serviceElement, "TargetReplicaSetSize") ?? 3;
                applicationManifestDefaultService.MinReplicaSetSize =
                    getIntValue(serviceElement, "MinReplicaSetSize") ?? 1;
                ...
                else
                {
                    ...
                    if (uniformInt64Partition != null)
                    {
                        ...
                        applicationManifestDefaultService.PartitioningLowLey =
                            getLongValue(uniformInt64Partition, "LowKey") ?? long.MinValue;
                        applicationManifestDefaultService.PartitioningHighLey =
                            getLongValue(uniformInt64Partition, "HighKey") ?? long.MaxValue;
                        applicationManifestDefaultService.PartitionCount =
                            getIntValue(uniformInt64Partition, "PartitionCount") ?? 10;
                    }

                    if (namedPartition != null)
                    {
                        applicationManifestDefaultService.ServicePartitionKind = ServicePartitionKind.Named;
                        applicationManifestDefaultService.NamedPartitions = namedPartition.Elements(xns + "Partition")
                            .Select(partition => partition.Attribute("Name").Value).ToArray();
                    }
