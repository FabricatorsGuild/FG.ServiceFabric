...
namespace FG.ServiceFabric.Testing.Setup
{
    ...
    {
        protected void Setup(MockFabricRuntime mockFabricRuntime, string applicationProjectPath, string applicationParametersPath = null)
        {
            ...
            var applicationManifest = ApplicationManifest.Load(applicationProject.ApplicationManifestPath, applicationParametersPath);
            ...
            foreach (var defaultService in applicationManifest.DefaultServices)
            {
                ...
                if (!(applicationProject.ServiceProjects.FirstOrDefault(s => s.Manifest.ServiceTypes.Any(t => t == defaultService.TypeName)) is ServiceProject serviceProject))
                {
            ...
            foreach (var serviceProject in serviceProjects)
            {
                ...
                var serviceManifestImport = applicationManifest.ServiceManifestImports.FirstOrDefault(import => import.Name.Equals(serviceProject.Manifest.Name));
                if (serviceManifestImport == null)
                {
                    throw new MockFabricSetupException($"Tried to load project {serviceProject.Name}/{serviceProject.Assembly.FullName} but did not find a ServiceManifestImport in the ApplicationManifest for {serviceProject.Manifest.Name}");
                }
                ...
                foreach (var serviceDefinition in serviceProject.ServiceDefinitions)
                {
                    ...
                    if (actorTypes.TryGetValue(actorName, out var actorType))
                    {
                        ...
                        this.SetupActor(serviceProject.FabricApplication, actorType, actorServiceType, serviceDefinition.Value);
                    }
                    else if (actorServiceTypes.TryGetValue(serviceName, out var actorServiceType))
                    {
                        ...
                        var likelyActorType = actorTypes.Values.FirstOrDefault(at => at.Name.StartsWith(actorTypeNameBeginning));
                        Console.WriteLine($"Create ActorService {serviceName} for type {actorName}/{likelyActorType?.FullName} - {serviceDefinition}");

                        this.SetupActor(serviceProject.FabricApplication, likelyActorType, actorServiceType, serviceDefinition.Value);
                    }
                    else if (statefulServiceTypes.TryGetValue(serviceName, out var statefulServiceType))
                    {
                        ...
                        this.SetupStatefulService(serviceProject.FabricApplication, statefulServiceType, serviceDefinition.Value);
                    }
                    else if (statelessServiceTypes.TryGetValue(serviceName, out var statelessServiceType))
                    {
                        ...
                        this.SetupStatelessService(serviceProject.FabricApplication, statelessServiceType, serviceDefinition.Value);
                    }
                }
        ...
        private ActorBase CreateActor(Type actorType, ActorService actorService, ActorId actorId)
        {
            ...
            foreach (var constructorParameter in maxArgsConstructor.GetParameters())
            {
                ...
                else
                {
                    ...
                    {
                        parameter = this.CreateStateSessionManager(actorService.Context);
                    }
                    else if (typeof(ISettingsProvider).IsAssignableFrom(parameterType))
                    {
                        parameter = this.CreateSettingsProvider(actorService.Context, actorService.GetType());
                    }

                    parameter = this.CreateActorParameter(actorType, actorService, actorId, parameterType, parameter);
                    if (parameter == null)
                        throw new MockServiceRuntimeException($"Trying to setup Actor {actorType.Name} {actorId.ToString()} and needs parameter of type {parameterType.GetFriendlyName()} but test class does not override CreateActorParameter or the override returns null");
        ...
        {
            var actorStateManagerFactory = this.CreateActorStateManagerFactory();

            fabricApplication.SetupActor(
                actorImplementationType: actorType,
                actorServiceImplementationType: actorServiceType,
                activator: (actorService, actorId) => this.CreateActor(actorType, actorService, actorId),
                createActorService: (context, information, stateProvider, stateManagerFactory) => this.CreateActorService(actorServiceType, information, context, stateProvider, stateManagerFactory),
                ...
                createActorStateProvider: this.CreateActorStateProvider,
        ...
        private void SetupStatelessService(MockFabricApplication fabricApplication, Type serviceType, MockServiceDefinition serviceDefinition)
        {
            fabricApplication.SetupService(
                serviceType,
                createService: context => this.CreateStatelessService(serviceType, context),
                serviceDefinition: serviceDefinition);
        }
        ...
        private void SetupStatefulService(MockFabricApplication fabricApplication, Type serviceType, MockServiceDefinition serviceDefinition)
        {
            fabricApplication.SetupService(
                serviceType,
                createService: (context, stateManagerReplica) => this.CreateStatefulService(serviceType, context, stateManagerReplica),
                createStateManager: () => this.CreateStateManager(serviceType, serviceDefinition),
                serviceDefinition: serviceDefinition);
        }
        ...
        private StatefulService CreateStatefulService(Type serviceType, StatefulServiceContext context, IReliableStateManagerReplica2 stateManagerReplica)
        {
            ...
            foreach (var constructorParameter in maxArgsConstructor.GetParameters())
            {
                ...
                else
                {
                    ...
                    {
                        parameter = this.CreateStateSessionManager(context);
                    }
                    else if (typeof(ISettingsProvider).IsAssignableFrom(parameterType))
                    {
                        parameter = this.CreateSettingsProvider(context, serviceType);
                    }

                    parameter = this.CreateServiceParameter(context, serviceType, parameterType, parameter);
                    if (parameter == null)
                        throw new MockServiceRuntimeException($"Trying to setup StatefulService {serviceType.Name} but test class does not override CreateServiceParameter or the override returns null for parameter {parameterType.GetFriendlyName()} on {context.ServiceName.ToString()}");
                    parameters.Add(parameter);
                }
            }
        ...
        private StatelessService CreateStatelessService(Type serviceType, StatelessServiceContext context)
        {
            ...
            foreach (var constructorParameter in maxArgsConstructor.GetParameters())
            {
                ...
                else
                {
                    ...
                    parameter = this.CreateServiceParameter(context, serviceType, parameterType, parameter);
                    if (parameter == null)
                        throw new MockServiceRuntimeException($"Trying to setup StatelessService {serviceType.Name} but test class does not override CreateServiceParameter or the override returns null for parameter {parameterType.GetFriendlyName()} on {context.ServiceName.ToString()}");
                    parameters.Add(parameter);
                }
            }
        ...
        private ActorService CreateActorService(Type actorServiceType, ActorTypeInformation actorTypeInformation, StatefulServiceContext context, IActorStateProvider stateProvider, Func<ActorBase, IActorStateProvider, IActorStateManager> stateManagerFactory)
        {
            ...
            foreach (var constructorParameter in maxArgsConstructor.GetParameters())
            {
                ...
                {
                    parameters.Add((Func<ActorService, ActorId, ActorBase>)((service, actorId) => this.CreateActor(actorTypeInformation.ImplementationType, service, actorId)));
                }
                else
                {
                    object parameter = null;
                    ...
                    {
                        parameter = this.CreateSettingsProvider(context, actorServiceType);
                    }

                    parameter = this.CreateActorServiceParameter(actorServiceType, parameterType, parameter);
                    if (parameter == null && !constructorParameter.IsOptional)
                        throw new MockServiceRuntimeException($"Trying to setup ActorService {actorServiceType.Name} but test class does not override CreateActorServiceParameter or the override returns null for parameter {parameterType.GetFriendlyName()} on {context.ServiceName.ToString()}");
                    parameters.Add(parameter);
        ...
        protected virtual object CreateActorParameter(Type actorType, ActorService actorService, ActorId actorId, Type parameterType, object defaultValue) { return defaultValue; }
        ...
        protected virtual object CreateActorServiceParameter(Type actorServiceType, Type parameterType, object defaultValue) { return defaultValue; }
        ...
        protected virtual object CreateServiceParameter(StatefulServiceContext context, Type serviceType, Type parameterType, object defaultValue) { return defaultValue; }

        protected virtual object CreateServiceParameter(StatelessServiceContext context, Type serviceType, Type parameterType, object defaultValue) { return defaultValue; }
    }
}