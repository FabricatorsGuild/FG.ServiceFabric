    ...
    public static class ObjectPropertyAssertions
    {
            ...
            string[] ignoredProperties = null)
        {
            var firstSetOfProperties = baseObject.GetType()
                ...
                .Where(pi =>
                    (ignoredProperties ?? Array.Empty<string>()).All(p =>
                        !string.Equals(p, pi.Name, StringComparison.OrdinalIgnoreCase)))
            ...
            var caseInsensitivePropertyNameTransforms =
                propertyNameTransforms.ToDictionary(kvp => kvp.Key, kvp => kvp.Value, StringComparer.OrdinalIgnoreCase);

            foreach (var firstSetPropertyKey in firstSetOfProperties.Keys)
            {
                if (caseInsensitivePropertyNameTransforms.TryGetValue(firstSetPropertyKey,
                        out var secondSetPropertyKey) == false)
        ...
        public static void CheckMatchingProperties(this object a, object b,
            Action<string, object, object> checkProperty, params string[] propertyNames)
        {
            var aProperties = a.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public)
                .ToDictionary(pi => pi.Name, pi => pi);
            var bProperties = b.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public)
                .ToDictionary(pi => pi.Name, pi => pi);
