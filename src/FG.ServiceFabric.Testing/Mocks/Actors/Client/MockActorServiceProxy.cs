...
namespace FG.ServiceFabric.Testing.Mocks.Actors.Client
{
	public class MockActorServiceProxy : MockServiceProxy
	{
		...
		protected override IEnumerable<IInterceptor> GetInterceptors(object target, Type serviceInterfaceType, IMockServiceProxyManager serviceProxyManager)
		{
		...
		private class ActorServiceInterceptor : IInterceptor, IInterceptorFilter
		{
			...
			{
				return method.DeclaringType == typeof(IActorService) && method.Name == nameof(IActorService.DeleteActorAsync);
			}
		}
	}
}