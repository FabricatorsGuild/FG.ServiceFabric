    ...
    public static class EventSourceValidator
    {
        public static void Call_from_tests_check_eventsources(Assembly actorOrServiceAssembly)
        {
            foreach (var eventSourceType in actorOrServiceAssembly.GetTypes().Where(type => type.IsSubclassOf(typeof(EventSource))))
            {
                ...
                if (eventSource == null) throw new NotImplementedException($"Expected EventSource {eventSourceType.Name} to expose a static accessor named 'Current' or 'Log'");
        ...
        {
            var accessorMethod = eventSourceType.GetField("Current", BindingFlags.GetField | BindingFlags.Static | BindingFlags.Public);
