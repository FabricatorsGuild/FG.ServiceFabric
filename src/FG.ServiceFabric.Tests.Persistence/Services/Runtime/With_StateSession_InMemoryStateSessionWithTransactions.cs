...
{
    namespace With_StateSession_InMemoryStateSessionWithTransactions
    {
            ...
            With_StateSession_All_Tests
        {
            private class TestRunnerBase<T> : TestRunnerWithFunc<T> where T : StatefulServiceDemoBase
            {
                ...
                public TestRunnerBase(Func<StatefulServiceContext, IStateSessionManager, T> createServiceFunc) : base(
                    createServiceFunc)
                {
                }
            ...
            public abstract class StateSession_transacted_scope : Runtime.StateSession_transacted_scope
            {
                ...
                {
                    return new InMemoryStateSessionManagerWithTransaction("testservice", Guid.NewGuid(), "range-0",
                        _state);
                }
            }
