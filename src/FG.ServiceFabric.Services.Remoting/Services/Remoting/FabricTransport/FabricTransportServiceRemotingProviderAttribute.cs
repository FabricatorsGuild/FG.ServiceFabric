...
{
    public class FabricTransportServiceRemotingProviderAttribute : Microsoft.ServiceFabric.Services.Remoting.
        FabricTransport.FabricTransportServiceRemotingProviderAttribute
    {
            ...
            IServiceRemotingCallbackClient callbackClient)
        {
            ...
            fabricTransportSettings.KeepAliveTimeout = GetKeepAliveTimeout(fabricTransportSettings.KeepAliveTimeout);
            var exceptionHandlers = ExceptionHandlerTypes?.Where(exceptionHandlerType =>
                    exceptionHandlerType.GetInterface(nameof(IExceptionHandler), false) != null)
                .Select(exceptionHandlerType => (IExceptionHandler) Activator.CreateInstance(exceptionHandlerType))
                .ToArray();
            return new FabricTransportServiceRemotingClientFactory(fabricTransportSettings, callbackClient, null,
        ...
        /// </returns>
        public override IServiceRemotingListener CreateServiceRemotingListener(ServiceContext serviceContext,
            IService serviceImplementation)
        {
            ...
            listenerSettings.KeepAliveTimeout = GetKeepAliveTimeout(listenerSettings.KeepAliveTimeout);
            var messageHandler =
                ServiceRemotingDispatcherAttribute.GetServiceRemotingDispatcher(serviceContext, serviceImplementation);

            return new FabricTransportServiceRemotingListener(serviceContext, messageHandler, listenerSettings);
        }

        private static FabricTransportRemotingListenerSettings GetDefaultFabricTransportListenerSettings(
        ...
        {
            return MaxMessageSize <= 0L ? maxMessageSize : MaxMessageSize;
        }

        private TimeSpan GetAndValidateOperationTimeout(TimeSpan operationTimeout)
        {
            return OperationTimeoutInSeconds <= 0L ? operationTimeout : TimeSpan.FromSeconds(OperationTimeoutInSeconds);
        }

        private TimeSpan GetKeepAliveTimeout(TimeSpan keepAliveTimeout)
        {
            return KeepAliveTimeoutInSeconds <= 0L ? keepAliveTimeout : TimeSpan.FromSeconds(KeepAliveTimeoutInSeconds);
        }
    }
}