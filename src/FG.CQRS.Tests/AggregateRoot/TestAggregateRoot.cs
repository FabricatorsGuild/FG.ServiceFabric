...
namespace FG.CQRS.Tests.AggregateRoot
{
    ...
    public class TestAggregateRoot : AggregateRoot<ITestEventBase>
    {
        ...
        public void Create(Guid aggregateRootId) { RaiseEvent(new TestCreatedEvent { AggregateRootId = aggregateRootId }); }

        public void AddEntity(string name) { RaiseEvent(new TestEntityL1AddedEvent(1) { Name = name }); }

        public void ForTestForceRaiseAnyEvent(ITestEventBase evt) { RaiseEvent(evt); }
        ...
        public class TestEntityL1 : Entity<TestAggregateRoot, ITestEntityL1EventBase>
        {
            ...
            public TestEntityL1(TestAggregateRoot aggregateRoot, int entityl1Id) : base(aggregateRoot)
            {
                ...
                RegisterEventAppliers()
                    ...
                    .For<ITestEntityL2EventBase>(e => EntityL2S.Single(x => x.Entityl2Id == e.Entityl2Id).ApplyEvent(e));
            }

            public void AddEntity(int value) { RaiseEvent(new TestEntityL2AddedEvent(Entityl1Id, 1) { Value = value }); }
