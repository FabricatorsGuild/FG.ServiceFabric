	...
	internal class ActorWithReminderDemo : FG.ServiceFabric.Actors.Runtime.ActorBase, IActorWithReminderDemo, IRemindable
	{
		...
		public async Task ReceiveReminderAsync(string reminderName, byte[] state, TimeSpan dueTime, TimeSpan period)
		{
			if (reminderName == _reminderName)
			{
				...
				Console.WriteLine($"Triggered reminder {reminderName} @ {DateTime.UtcNow.ToLocalTime().ToShortTimeString()} with {data.ValueToTickDown} started {(DateTime.UtcNow - data.StartedTime).TotalSeconds} s. ago");
