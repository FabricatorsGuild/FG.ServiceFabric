...
namespace FG.ServiceFabric.Actors
{
	public class ActorReference
	{
		...
		/// <summary>
		/// Creates an <see cref="T:Microsoft.ServiceFabric.Actors.Client.ActorProxy" /> that implements an actor interface for the actor using the
		///     <see cref="M:Microsoft.ServiceFabric.Actors.Client.ActorProxyFactory.CreateActorProxy(System.Type,System.Uri,Microsoft.ServiceFabric.Actors.ActorId,System.String)" />
		/// method.
		/// </summary>
		/// <param name="actorInterfaceType">Actor interface for the created <see cref="T:Microsoft.ServiceFabric.Actors.Client.ActorProxy" /> to implement.</param>
		...
		{
			return this.CallGenericMethod(nameof(BindInternal), new[] {actorInterfaceType}, this.ServiceUri, this.ActorId, this.ListenerName);
		}

		public static implicit operator ActorReference(Microsoft.ServiceFabric.Actors.ActorReference actorReference)
		{
			return new ActorReference() {ActorId = actorReference.ActorId, ListenerName = actorReference.ListenerName, ServiceUri = actorReference.ServiceUri};
		}

		private object BindInternal<TActorInterfaceType>(Uri serviceUri, ActorId actorId, string listenerName) where TActorInterfaceType : IActor
		{
