...
namespace FG.ServiceFabric.Actors.Runtime.Reminders
{
        ...
        where T : ReminderDataBase<T>
    {
        ...
        public static T Deserialize(byte[] data,
            ReminderDataSerializationType type = ReminderDataSerializationType.Binary)
        {
            if (type == ReminderDataSerializationType.Binary)
            ...
            using (var memStream = new MemoryStream(data))
            {
                ...
                catch (Exception ex)
                {
                    throw new ArgumentException(
                        $"The byte array is not a serialized state of {typeof(T)}, {ex.Message}");
                }
            }
        }
        ...
        {
            var serializedObject = Encoding.UTF8.GetString(data);
