            ...
            CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            foreach (var stateChange in stateChanges)
                if (stateChange.ChangeKind == StateChangeKind.Add)
                {
                    ...
                    if (stateName == StateObjectKey)
                    {
                        actorStateChanges.Add(new ActorStateChange(stateName, stateChange.Type, stateChange.Value,
                            StateChangeKind.Add));
                        if (await ContainsStateAsync(actorId, StateVersionKey, cancellationToken))
                            actorStateChanges.Add(new ActorStateChange(StateVersionKey, typeof(int),
                                _migrationContainer.CurrentState(),
                ...
                else if (stateChange.ChangeKind == StateChangeKind.Update)
                {
                    ...
                    if (stateName == StateObjectKey)
                    {
                        actorStateChanges.Add(
                            new ActorStateChange(stateName, stateChange.Type, stateChange.Value,
                                StateChangeKind.Update));
                        if (await ContainsStateAsync(actorId, StateVersionKey, cancellationToken))
                            actorStateChanges.Add(new ActorStateChange(StateVersionKey, typeof(int),
                                _migrationContainer.CurrentState(),
