            ...
            foreach (var stateChange in stateChanges)
            {
                if (stateChange.ChangeKind == StateChangeKind.Add)
                {
                    ...
                    if (stateName == StateObjectKey)
                    {
                        ...
                        {
                            actorStateChanges.Add(new ActorStateChange(StateVersionKey, typeof(int), _migrationContainer.CurrentState(), StateChangeKind.Update));
                        }
                        else
                        {
                            actorStateChanges.Add(new ActorStateChange(StateVersionKey, typeof(int), _migrationContainer.CurrentState(), StateChangeKind.Add));
                        }
                    }
                }
                else if (stateChange.ChangeKind == StateChangeKind.Update)
                {
                    ...
                    if (stateName == StateObjectKey)
                    {
                        actorStateChanges.Add(new ActorStateChange(stateName, stateChange.Type, stateChange.Value, StateChangeKind.Update));
                        if ((await ContainsStateAsync(actorId, StateVersionKey, cancellationToken)))
                        {
                            actorStateChanges.Add(new ActorStateChange(StateVersionKey, typeof(int), _migrationContainer.CurrentState(), StateChangeKind.Update));
                        }
                ...
                else if (stateChange.ChangeKind == StateChangeKind.Remove)
                {
                    ...
                    if (stateName == StateObjectKey)
                    {
                        actorStateChanges.Add(new ActorStateChange(stateName, stateChange.Type, stateChange.Value, StateChangeKind.Remove));
                        if ((await ContainsStateAsync(actorId, StateVersionKey, cancellationToken)))
                        {
                            actorStateChanges.Add(new ActorStateChange(StateVersionKey, typeof(int), _migrationContainer.CurrentState(), StateChangeKind.Remove));
                        }
                    }
                }
            }
