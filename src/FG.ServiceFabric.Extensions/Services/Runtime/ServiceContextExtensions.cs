...
namespace FG.ServiceFabric.Services.Runtime
{
    public static class ServiceContextExtensions
    {
        ...
        public static Task<IList<Int64RangePartitionInformation>> GetServicePartitionKeysAsync(
            IPartitionEnumerationManager partitionEnumerationManager, ServiceContext context)
        {
            return GetServicePartitionKeysAsync(partitionEnumerationManager, context.ServiceName);
        }

        public static async Task<IList<Int64RangePartitionInformation>> GetServicePartitionKeysAsync(
            IPartitionEnumerationManager partitionEnumerationManager, Uri serviceName)
        {
            for (var i = 0; i < MaxQueryRetryCount; i++)
            {
                try
                {
                    ...
                    IList<Int64RangePartitionInformation>
                        partitionKeys = new List<Int64RangePartitionInformation>(partitionList.Count);
            ...
            throw new TimeoutException(
                "Retry timeout is exhausted and creating representative partition clients wasn't successful");
        }
    }
}