...
namespace FG.ServiceFabric.Data
{
	...
	{
		public static async Task<TValue> GetSimpleValueAsync<TKey, TValue>(this IReliableDictionary<TKey, TValue> dictionary,
		...
		private static async Task<IEnumerable<object>> EnumerateState<T, T2>(IReliableStateManager stateManager,
			IReliableDictionary<T, T2> collection, CancellationToken cancellationToken) where T : IComparable<T>, IEquatable<T>
		{
