...
namespace FG.ServiceFabric.Services.Runtime.StateSession.ReliableState
{
    ...
    public class ReliableStateSessionQueue<T> : ReliableStateSessionReadOnlyQueue<T>, IStateSessionQueue<T>
    {
        ...
        public ReliableStateSessionQueue(IReliableConcurrentQueue<T> reliableConcurrentQueue) : base(
        ...
        public Task EnqueueAsync(T value, IValueMetadata metadata,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            throw new NotImplementedException();
        }

        public Task<ConditionalValue<T>> DequeueAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            throw new NotImplementedException();
        }
    ...
    public class ReliableStateSessionDictionary<T> : IStateSessionDictionary<T>
    {
        ...
        public ReliableStateSessionDictionary(IReliableDictionary2<string, T> reliableDictionary, bool readOnly)
        {
        ...
        public Task<ConditionalValue<T>> TryGetValueAsync(string key,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            throw new NotImplementedException();
        }
        ...
        public Task SetValueAsync(string key, T value, IValueMetadata metadata,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            throw new NotImplementedException();
        }
        ...
        public Task<IAsyncEnumerable<KeyValuePair<string, T>>> CreateEnumerableAsync()
        {
            throw new NotImplementedException();
        }
    ...
    public class ReliableStateSessionManager : IStateSessionManager, IStateSessionWritableManager
    {
        ...
        private readonly ConcurrentDictionary<string, Type> _reliableStateTypes =
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                reliableDictionary2 = await StateManager.GetOrAddAsync<IReliableDictionary2<string, T>>(schema);
                _reliableDictionaries[schema] = reliableDictionary2;
            }, 3, TimeSpan.FromSeconds(1), cancellationToken);

            return new ReliableStateSessionDictionary<T>(reliableDictionary2, false);
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            await ExecutionHelper.ExecuteWithRetriesAsync(async cn =>
            {
                reliableConcurrentQueue = await StateManager.GetOrAddAsync<IReliableConcurrentQueue<T>>(schema);
        ...
        async Task<IStateSessionDictionary<T>> IStateSessionWritableManager.OpenDictionary<T>(string schema,
        ...
        async Task<IStateSessionQueue<T>> IStateSessionWritableManager.OpenQueue<T>(string schema,
            CancellationToken cancellationToken)
        {
            cancellationToken = cancellationToken == default(CancellationToken)
                ? CancellationToken.None
            ...
            await ExecutionHelper.ExecuteWithRetriesAsync(async cn =>
            {
                reliableQueue = await StateManager.GetOrAddAsync<IReliableConcurrentQueue<T>>(schema);
        ...
        {
            var isReadOnly = readOnly & stateSessionObjects.All(o => o.IsReadOnly);
            return new ReliableStateSession(this, readOnly);
        }

        private static Type GetReliableStateInterfaceType(Type implementationType)
        {
            var genericInterfaceTypes = implementationType.GetInterfaces().Where(i => i.IsGenericType)
            ...
            foreach (var genericInterfaceType in genericInterfaceTypes)
            {
                ...
                if (genericInterfaceType == typeof(IReliableConcurrentQueue<>))
                    return typeof(IReliableConcurrentQueue<>);
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var stateEnumerator = StateManager.GetAsyncEnumerator();
        ...
        private sealed class ReliableStateSession : IStateSession
        {
            ...
            public ReliableStateSession(ReliableStateSessionManager sessionManager, bool readOnly)
            {
                IsReadOnly = readOnly;
            ...
            public async Task<bool> Contains<T>(string schema, string key,
                CancellationToken cancellationToken = default(CancellationToken))
            {
                var value = false;
                try
                {
                    ...
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn =>
                        {
                            value = await reliableDictionary2.ContainsKeyAsync(GetTransaction(), key,
            ...
            public async Task<bool> Contains(string schema, string key,
                CancellationToken cancellationToken = default(CancellationToken))
            {
                try
                {
                    ...
                    var reliableDictionary2 =
                        this.CallGenericMethod(methodNameGetDictionary, new[] {valueType}, schema);
                    ...
                    var contains = await ExecutionHelper.ExecuteWithRetriesAsync(async cn =>
                    {
                        var task = (Task<bool>) reliableDictionary2
                            .CallGenericMethod(methodNameContainsKeyAsync, new[] {valueType}, GetTransaction(), key,
                ...
                CancellationToken cancellationToken = default(CancellationToken))
            {
                ...
                try
                {
                    ...
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn =>
                        {
                            value = await reliableDictionary2.TryGetValueAsync(GetTransaction(), key,
                                TimeSpan.FromSeconds(1),
                                cancellationToken);
                        }, 3, TimeSpan.FromSeconds(1), cancellationToken);
                    return value;
                ...
                CancellationToken cancellationToken = new CancellationToken())
            {
                return FindByKeyPrefixInternalAsync(schema, keyPrefix, maxNumResults,
                    async (stateSessionManager, schema2) =>
                    {
                        var reliableDictionary2 = new ReliableDictionaryNonGenericWrapper(_sessionManager, schema);
                        return await reliableDictionary2.Initialize(cancellationToken);
                    },
                    async (reliableDictionary2, transaction, cn) =>
                        await ((ReliableDictionaryNonGenericWrapper) reliableDictionary2)
                            .CreateKeyEnumerableAsync<string>(transaction,
...
//					cancellationToken: cancellationToken);
                ...
                CancellationToken cancellationToken = new CancellationToken())
            {
                ...
                var stateEnumerator = _sessionManager.StateManager.GetAsyncEnumerator();
                while (await stateEnumerator.MoveNextAsync(cancellationToken))
                {
                    var currentState = stateEnumerator.Current;
                    var currentStateSchema = GetSchemaFromUrn(currentState.Name);
                ...
                CancellationToken cancellationToken = default(CancellationToken))
            {
                try
                {
                    ...
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn =>
                        {
                            await reliableDictionary2.SetAsync(GetTransaction(), key, value, TimeSpan.FromSeconds(3),
                                cancellationToken);
                        }, 3, TimeSpan.FromSeconds(1), cancellationToken);
                    _needsCommit = true;
                }
            ...
            public async Task RemoveAsync<T>(string schema, string key,
                CancellationToken cancellationToken = default(CancellationToken))
            {
                try
                {
                    ...
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn =>
                        {
                            await reliableDictionary2.TryRemoveAsync(GetTransaction(), key, TimeSpan.FromSeconds(3),
                                cancellationToken);
                        }, 3, TimeSpan.FromSeconds(1), cancellationToken);
                    _needsCommit = true;
                }
            ...
            public async Task<long> GetDictionaryCountAsync<T>(string schema, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    long value = 0;
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn => { value = await dictionary.GetCountAsync(GetTransaction()); }, 3,
                        TimeSpan.FromSeconds(1),
                        cancellationToken);
                    _needsCommit = true;
                ...
                CancellationToken cancellationToken = default(CancellationToken))
            {
                try
                {
                    ...
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn =>
                        {
                            await queue.EnqueueAsync(GetTransaction(), value, TimeSpan.FromSeconds(3),
                                cancellationToken);
                        },
                        3, TimeSpan.FromSeconds(1), cancellationToken);
                    _needsCommit = true;
                ...
                CancellationToken cancellationToken = default(CancellationToken))
            {
                try
                {
                    ...
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn =>
                        {
                            value = await queue.TryDequeueAsync(GetTransaction(), TimeSpan.FromSeconds(3),
                                cancellationToken);
                        }, 3, TimeSpan.FromSeconds(1), cancellationToken);
                    _needsCommit = true;
                ...
                CancellationToken cancellationToken = default(CancellationToken))
            {
                try
                {
                    ...
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn =>
                        {
                            value = await queue.TryPeekAsync(GetTransaction(), TimeSpan.FromSeconds(3),
                                cancellationToken);
                        },
                        3, TimeSpan.FromSeconds(1), cancellationToken);
                    _needsCommit = true;
            ...
            public async Task<long> GetEnqueuedCountAsync<T>(string schema, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    long value = 0;
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async cn => { value = await queue.GetCountAsync(GetTransaction()); }, 3,
                        TimeSpan.FromSeconds(1),
                        cancellationToken);
                    _needsCommit = true;
            ...
            private async Task<FindByKeyPrefixResult> FindByKeyPrefixInternalAsync(string schema, string keyPrefix,
                int maxNumResults,
                Func<IStateSessionManager, string, Task<object>> reliableDictionaryFactory,
                Func<object, ITransaction, CancellationToken, Task<IAsyncEnumerable<string>>>
                    createKeyEnumerableAsyncFactory,
                ContinuationToken continuationToken = null,
                CancellationToken cancellationToken = new CancellationToken())
            {
                var result = new List<string>();
                ...
                try
                {
                    ...
                    {
                        var keyEnumerable =
                            await createKeyEnumerableAsyncFactory(reliableDictionary2, GetTransaction(),
                                cancellationToken);
                    ...
                    return new FindByKeyPrefixResult
                    {
                        ...
                        ContinuationToken = nextContinuationToken != null
                            ? new ContinuationToken(nextContinuationToken)
                            : null
                    };
                }
                catch (Exception ex)
            ...
            private class ReliableDictionaryNonGenericWrapper
            {
                ...
                public async Task<ReliableDictionaryNonGenericWrapper> Initialize(
                    CancellationToken cancellationToken = default(CancellationToken))
                {
                    ...
                    _reliableDictionaryType =
                        typeof(IReliableDictionary<,>).MakeGenericType(typeof(string), _valueType);
                    _reliableDictionary2Type =
                        typeof(IReliableDictionary2<,>).MakeGenericType(typeof(string), _valueType);

                    var methodNameGetDictionary = nameof(GetDictionary);
                    _reliableDictionary2 =
                        (IReliableState) _sessionManager.CallGenericMethod(methodNameGetDictionary, new[] {_valueType},
                            _schema);

                    _methodNameCreateKeyEnumerableAsync =
                        nameof(IReliableDictionary2<string, string>.CreateKeyEnumerableAsync);
                ...
                public Task<IAsyncEnumerable<TKey>> CreateKeyEnumerableAsync<TKey>(ITransaction txn)
                {
                    var methodInfo = _reliableDictionary2Type
                        .GetMethod(_methodNameCreateKeyEnumerableAsync,
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null,
                            new[] {typeof(ITransaction)}, null);

                    var task = (Task<IAsyncEnumerable<TKey>>) methodInfo.Invoke(_reliableDictionary2,
                        new object[] {txn});
                    return task;
                }
                    ...
                    EnumerationMode enumerationMode)
                {
                    var methodInfo = _reliableDictionary2Type
                        .GetMethod(
                            _methodNameCreateKeyEnumerableAsync,
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null,
                            new[] {typeof(ITransaction), typeof(EnumerationMode)}, null);

                    var task = (Task<IAsyncEnumerable<TKey>>) methodInfo.Invoke(_reliableDictionary2,
                        new object[] {txn, enumerationMode});
                    return task;
                }

                public Task<IAsyncEnumerable<TKey>> CreateKeyEnumerableAsync<TKey>(ITransaction txn,
                    EnumerationMode enumerationMode, TimeSpan timeout,
                    CancellationToken cancellationToken)
                {
                    var methodInfo = _reliableDictionary2Type
                        .GetMethod(_methodNameCreateKeyEnumerableAsync,
                            ...
                            new[]
                            {
                                typeof(ITransaction), typeof(EnumerationMode), typeof(TimeSpan),
                                typeof(CancellationToken)
                            }, null);

                    var task = (Task<IAsyncEnumerable<TKey>>) methodInfo.Invoke(_reliableDictionary2,
                        new object[] {txn, enumerationMode, timeout, cancellationToken});
                    return task;
                }
                ...
                public Task SetAsync<TKey>(ITransaction tx, TKey key, object value)
                {
                    var methodInfo = _reliableDictionaryType
                        ...
                        .FirstOrDefault(method =>
                            method.Name.Equals(_methodNameSetAsync) && method.GetParameters().Length == 3);

                    var task = (Task) methodInfo.Invoke(_reliableDictionary2, new[] {tx, key, value});
                    return task;
                }

                public Task SetAsync<TKey>(ITransaction tx, TKey key, object value, TimeSpan timeout,
                    CancellationToken cancellationToken)
                {
                    var methodInfo = _reliableDictionaryType
                        .GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                        .FirstOrDefault(method =>
                            method.Name.Equals(_methodNameSetAsync) && method.GetParameters().Length == 5);

                    var task = (Task) methodInfo.Invoke(_reliableDictionary2,
                        new[] {tx, key, value, timeout, cancellationToken});
                    return task;
                }
                ...
                public Task TryRemoveAsync<TKey>(ITransaction tx, TKey key)
                {
                    var methodInfo = _reliableDictionaryType
                        .GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                        .FirstOrDefault(method =>
                            method.Name.Equals(_methodNameRemoveAsync) && method.GetParameters().Length == 2);

                    var task = (Task) methodInfo.Invoke(_reliableDictionary2, new object[] {tx, key});
                    return task;
                }

                public Task TryRemoveAsync<TKey>(ITransaction tx, TKey key, TimeSpan timeout,
                    CancellationToken cancellationToken)
                {
                    var methodInfo = _reliableDictionaryType
                        .GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                        .FirstOrDefault(method =>
                            method.Name.Equals(_methodNameRemoveAsync) && method.GetParameters().Length == 4);

                    var task = (Task) methodInfo.Invoke(_reliableDictionary2,
                        new object[] {tx, key, timeout, cancellationToken});
                    return task;
                }
            }
        }
