...
namespace FG.ServiceFabric.Actors.Runtime
{
    public partial class StateSessionActorStateProvider : IActorStateProvider, IActorStateProviderQueryable
    {
        ...
        {
            _actorTypeInformation = actorTypeInformation;
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            using (var session = _stateSessionManager.CreateSession())
            {
                ...
                var result = await session.FindByKeyPrefixAsync<T>(schema, null, numItemsToReturn, continuationToken,
                    cancellationToken);

                var items = result.Items.Select(item =>
                        new KeyValuePair<ActorId, T>(ActorSchemaKey.TryGetActorIdFromSchemaKey(item.Key), item.Value))
                    .ToArray();
                ...
                foreach (var actorStateChange in stateChanges)
                {
                    ...
                    switch (actorStateChange.ChangeKind)
                    {
                        ...
                        case StateChangeKind.Update:
                            ...
                            await session.SetValueAsync(key.Schema, key.Key, actorStateChange.Type,
                                actorStateChange.Value, metadata,
                                cancellationToken);
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            try
            {
                ...
                {
                    // e.g.: servicename_partition1_ACTORREMINDER_G:A4F3A8FC-801E-4940-8993-98CB6D7BCEF9-wakeupcall
                    var reminder = await session.TryGetValueAsync<ActorReminderData>(
                        ActorReminderStateKey.ActorReminderSchemaName,
                    ...
                    if (reminder.HasValue)
                    {
                        ...
                        var reminderCompleted = await session
                            .TryGetValueAsync<ActorReminderCompletedData>(
                                ActorReminderCompletedStateKey.ActorReminderCompletedSchemaName, reminderKey,
                                cancellationToken);
