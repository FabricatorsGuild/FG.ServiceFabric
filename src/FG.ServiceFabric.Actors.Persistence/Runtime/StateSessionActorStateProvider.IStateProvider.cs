...
namespace FG.ServiceFabric.Actors.Runtime
{
    public partial class StateSessionActorStateProvider : IStateProvider
    {
        ...
        {
            var fabricReplicator = partition.CreateReplicator(this, GetReplicatorSettings());
            _replicator = fabricReplicator.StateReplicator2;
            _servicePartition = partition;
        ...
        private ReplicatorSettings GetReplicatorSettings()
        {
            ...
            return FG.ServiceFabric.Actors.Runtime.ActorStateProviderHelper.GetActorReplicatorSettings(
                _initParams.CodePackageActivationContext,
                _actorTypeInformation.ImplementationType);
        }
        ...
        Task IStateProviderReplica.ChangeRoleAsync(ReplicaRole newRole, CancellationToken cancellationToken)
        {
            switch (newRole)
            {
                ...
                case ReplicaRole.ActiveSecondary:
                    ...
                    if (_currentRole == ReplicaRole.Primary)
        ...
        private void StartSecondaryPumpOperation(bool isCopy)
        {
            Task.Run(
                async () =>
                {
                    var operationStream = GetOperationStream(isCopy);
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            _servicePartition.ReportFault(FaultType.Transient);
                        }
                    }
                    catch (Exception)
                    {
        ...
        {
            return isCopy ? _replicator.GetCopyStream() : _replicator.GetReplicationStream();
        }
