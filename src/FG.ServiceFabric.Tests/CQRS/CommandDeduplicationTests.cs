...
namespace FG.ServiceFabric.Tests.CQRS
{
    ...
    public class IdempotentCommandHandlingTests
    {
        ...
        public void Setup()
        {
            _fabricRuntime = new MockFabricRuntime(Guid.NewGuid().ToString());
        ...
        [Test]
        public async Task Executing_same_command_adding_count_twice_idempotently_performing_sync_action_results_in_count_being_1()
        {
            var id = new ActorId(Guid.NewGuid().ToString());
            await _fabricRuntime.ActorProxyFactory
                .CreateActorProxy<ITestActor>(id)
            ...
            await _fabricRuntime.ActorProxyFactory
                .CreateActorProxy<ITestActor>(id)
                .AddCountAsyncPerformingSyncAction(_command); //second time, note its same command, hence same commandid.
            var count = await _fabricRuntime.ActorProxyFactory
                .CreateActorProxy<ITestActor>(id)
                .GetCountAsync();
            count.Should().Be(1);
        ...
        [Test]
        public async Task Executing_same_command_adding_count_twice_idempotently_performing_async_func_results_in_count_being_1()
        {
            var id = new ActorId(Guid.NewGuid().ToString());
            await _fabricRuntime.ActorProxyFactory
                .CreateActorProxy<ITestActor>(id)
        ...
        [Test]
        public async Task Executing_same_command_adding_count_twice_idempotently_performing_async_func_are_returning_same_value_as_in_first_execution()
        {
            var id = new ActorId(Guid.NewGuid().ToString());
            ...
            var returnValue2 = await _fabricRuntime.ActorProxyFactory
                .CreateActorProxy<ITestActor>(id)
                .AddCountAsyncPerformingAsyncFuncReturningValue(_command); //second time, note its same command, hence same commandid.
            ...
            return await CommandDeduplicationHelper.ProcessOnceAsync(
                async token =>
                {
                    await ExecutionHelper.ExecuteWithRetriesAsync(
                        async ct =>
                        {
                            await StateManager.AddOrUpdateStateAsync("MyState", 1, (s, i) => ++i, ct);
                        },
                        maxRetries: 3,
            ...
            await CommandDeduplicationHelper.ProcessOnceAsync(
                () =>
                {
                    ExecutionHelper.ExecuteWithRetriesAsync(
                       ...
                       maxRetries: 3,
                       retryDelay: 1.Seconds(),
                       userCancellationToken: CancellationToken.None).GetAwaiter().GetResult();
                },
                command: command,
                stateManager: StateManager,
                cancellationToken: CancellationToken.None);
