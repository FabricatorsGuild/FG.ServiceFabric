...
namespace FG.ServiceFabric.Actors.Runtime
{
        ...
        where TAggregateRoot : class, IEventStored, new()
    {
        ...
        protected TAggregateRoot DomainState;
            ...
            : base(actorService, actorId)
        {
            ...
            OutboundMessageChannel = new OutboundReliableMessageChannel(
                StateManager,
                ActorProxyFactory,
                outboundMessageChannelLoggerFactory,
                messageDrop,
                actorBinder
            );
        ...
        public async Task RaiseDomainEventAsync<TDomainEvent>(TDomainEvent domainEvent)
        ...
        protected override Task OnActivateAsync()
        {
            if (_outboundMessageChannelTimer == null)
                _outboundMessageChannelTimer = RegisterTimer(
                    async _ => { await OutboundMessageChannel.ProcessQueueAsync(CancellationToken.None); }, null,
        ...
        protected Task<TAggregateRoot> GetAndSetDomainAsync()
        {
            ...
            {
                var eventStream = await StateManager.GetOrAddStateAsync(EventStreamStateKey, new TEventStream(), ct);
        ...
        protected Task StoreDomainEventAsync(IDomainEvent domainEvent)
        {
            return ExecutionHelper.ExecuteWithRetriesAsync(async ct =>
            {
                ...
                await StateManager.SetStateAsync(EventStreamStateKey, eventStream, ct);
            }, 3, TimeSpan.FromSeconds(1), CancellationToken.None);
        }
        ...
        protected Task SendMessageAsync<TActorInterface>(ReliableMessage message, ActorId actorId,
            CancellationToken cancellationToken, string applicationName = null,
            string serviceName = null, string listerName = null)
            where TActorInterface : IReliableMessageReceiverActor
        {
            return OutboundMessageChannel.SendMessageAsync<TActorInterface>(message, actorId, cancellationToken,
                applicationName,
                serviceName,
                listerName);
        }
