...
namespace FG.ServiceFabric.Tests.StatefulServiceDemo
{
    ...
    public abstract class StatefulServiceDemoBase : StatefulService
    {
        ...
        {
            RunAsyncLoop?.Invoke(this, new RunAsyncLoopEventArgs {Context = Context, Iteration = iteration});
        }
    ...
    namespace With_multiple_states
    {
        ...
        public sealed class StatefulServiceDemo : StatefulServiceDemoBase, IStatefulServiceDemo
        {
            ...
            public async Task RunWork()
            {
                var cancellationToken = CancellationToken.None;
                var myDictionary =
                    await _stateSessionManager.Writable.OpenDictionary<long>("myDictionary", cancellationToken);
                var dictionary2 =
                    await _stateSessionManager.Writable.OpenDictionary<string>("myDictionary2", cancellationToken);
    ...
    namespace With_polymorphic_array_state
    {
        ...
        public sealed class StatefulServiceDemo : StatefulServiceDemoBase, IStatefulServiceDemo
        {
            ...
            public async Task<ArrayState> GetStateAsync(CancellationToken cancellationToken)
            {
                ...
                {
                    var stateValue =
                        await session.GetValueAsync<ArrayState>("myDictionary2", "theValue", cancellationToken);
