    ...
    internal static class DocumentClientExtensions
    {
            ...
            IDocumentDbStateSessionManagerLogger logger)
        {
            ...
            var documentCollection =
                readDocumentCollectionFeedAsync.FirstOrDefault(x => x.Id == collection.CollectionName);
        ...
        public static async Task DestroyCollection(this IDocumentClient @this, string databaseId, string collectionId)
        {
            try
            {
                ...
                var readDocumentCollectionFeedAsync =
                    await @this.DeleteDocumentCollectionAsync(
                        UriFactory.CreateDocumentCollectionUri(databaseId, collectionId));
            }
            catch (DocumentClientException ex)
            {
                Console.WriteLine(
                    $"Failed to destroy collection {collectionId} in database {databaseId} - {ex.Message}");
