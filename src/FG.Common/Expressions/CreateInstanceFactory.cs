...
namespace FG.Common.Expressions
{
    ...
    public static class CreateInstanceFactory
    {
        ...
        {
            var constructorInfo =
                typeToInstantiate.GetConstructors().FirstOrDefault(c => c.GetParameters().Length == 0);

            if (constructorInfo == null)
                throw new Exception("Type " + typeToInstantiate.FullName +
                                    " does not contain a parameterless constructor.");

            var currentExpression = Expression.Lambda(typeof(Func<>).MakeGenericType(typeToReturn),
