    ...
    public class ActorProxyFactory : ServiceProxyFactoryBase, IActorProxyFactory
    {
            ...
            where TServiceInterface : IService
        {
            ...
            var proxy = GetInnerServiceProxyFactory(typeof(TServiceInterface))
                .CreateServiceProxy<TServiceInterface>(serviceUri, new ServicePartitionKey(partitionKey),
                    TargetReplicaSelector.Default, listenerName);
            UpdateRequestContext(serviceUri);
            return proxy;
        }
            ...
            Type actorInterfaceType)
        {
            ...
            var actorServiceMethodDispatcher = GetOrDiscoverServiceMethodDispatcher(typeof(IActorService));
