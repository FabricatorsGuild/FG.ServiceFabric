...
namespace FG.ServiceFabric.Actors.Client
{
	public class ActorProxyFactory : ServiceProxyFactoryBase, IActorProxyFactory
	{
		...
		private static readonly ConcurrentDictionary<Type, MethodDispatcherBase> ActorMethodDispatcherMap =
			...
			string serviceName = null, string listenerName = null) where TActorInterface : IActor
		{
			...
			var proxy = GetInnerActorProxyFactory(null, typeof(TActorInterface))
				.CreateActorProxy<TActorInterface>(actorId, applicationName, serviceName, listenerName);
			...
			where TActorInterface : IActor
		{
			GetOrDiscoverActorMethodDispatcher(typeof(TActorInterface));
			var proxy = GetInnerActorProxyFactory(null, typeof(TActorInterface))
				.CreateActorProxy<TActorInterface>(serviceUri, actorId, listenerName);
			UpdateRequestContext(serviceUri);
			return proxy;
		}
			...
			string listenerName = null) where TServiceInterface : IService
		{
			...
			var proxy = GetInnerServiceProxyFactory(typeof(TServiceInterface)).CreateServiceProxy<TServiceInterface>(serviceUri,
				new ServicePartitionKey(partitionKey), TargetReplicaSelector.Default, listenerName);
			UpdateRequestContext(serviceUri);
			return proxy;
		}
		...
		private IServiceRemotingClientFactory CreateServiceRemotingClientFactory(
			IServiceRemotingCallbackClient serviceRemotingCallbackClient, Type serviceInterfaceType, Type actorInterfaceType)
		{
			...
			var actorServiceMethodDispatcher =
				base.GetOrDiscoverServiceMethodDispatcher(typeof(Microsoft.ServiceFabric.Actors.IActorService));
		...
		private static MethodDispatcherBase GetActorMethodInformation(Type actorInterfaceType)
		{
			...
			var methodDispatcherBase =
				getOrCreateMethodDispatcher?.Invoke(null, new object[] {actorInterfaceType}) as MethodDispatcherBase;
