...
namespace FG.ServiceFabric.Actors.Remoting.Runtime
{
	public static class ActorServiceRemotingDispatcherExtensions
	{
		public static string GetMethodDispatcherMapName(
			this Microsoft.ServiceFabric.Actors.Remoting.V1.Runtime.ActorServiceRemotingDispatcher that,
			int interfaceId, int methodId)
		{
			try
			{
				var actorServiceFieldInfo =
				...
				var getDispatcherMethodInfo =
					methodDispatcherMap?.GetType().GetMethod("GetDispatcher", new Type[] {typeof(int), typeof(int)});
		...
		public static string GetMethodDispatcherMapName(
			this Microsoft.ServiceFabric.Services.Remoting.Builder.MethodDispatcherBase that, int interfaceId, int methodId)
		{
