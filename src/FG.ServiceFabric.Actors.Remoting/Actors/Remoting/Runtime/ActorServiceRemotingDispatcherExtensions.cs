            ...
            int interfaceId, int methodId)
        {
            try
            {
                var actorServiceFieldInfo =
                    typeof(Microsoft.ServiceFabric.Actors.Remoting.V1.Runtime.ActorServiceRemotingDispatcher).GetField(
                        "actorService",
                ...
                var getDispatcherMethodInfo =
                    methodDispatcherMap?.GetType().GetMethod("GetDispatcher", new[] {typeof(int), typeof(int)});
