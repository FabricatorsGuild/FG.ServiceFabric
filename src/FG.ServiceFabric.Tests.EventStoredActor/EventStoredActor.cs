...
namespace FG.ServiceFabric.Tests.EventStoredActor
{
		...
		IHandleDomainEvent<ChildAddedEvent>
	{
		...
		{
			await SendMessageAsync<IIndexActor>(
				ReliableMessage.Create(new IndexCommand {PersonId = domainEvent.AggregateRootId}), new ActorId("Index"),
				CancellationToken.None, applicationName: "FG.ServiceFabric.Tests.Application");
			await StoreDomainEventAsync(domainEvent);
		}
